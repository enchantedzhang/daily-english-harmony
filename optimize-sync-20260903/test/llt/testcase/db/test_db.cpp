...
TEST_F(TestDB, DISABLED_test_subtable_entry_iterator_open_benchmark)
{
    const uint32_t entryCount = []() {
        const char *value = std::getenv("BSS_SUBTABLE_BENCH_ENTRIES");
        return value == nullptr ? 50000U : static_cast<uint32_t>(std::stoul(value));
    }();
    const uint32_t rounds = []() {
        const char *value = std::getenv("BSS_SUBTABLE_BENCH_ROUNDS");
        return value == nullptr ? 15U : static_cast<uint32_t>(std::stoul(value));
    }();
    const std::string mode = []() {
        const char *value = std::getenv("BSS_SUBTABLE_BENCH_MODE");
        return value == nullptr ? std::string("fresh") : std::string(value);
    }();
    ASSERT_GT(entryCount, 0U);
    ASSERT_GT(rounds, 0U);

    std::array<uint8_t, 16> priKeyBytes = { 's', 'u', 'b', 't', 'a', 'b', 'l', 'e', '-', 'b', 'e', 'n', 'c', 'h', 0,
                                            1 };
    BinaryData priKey(priKeyBytes.data(), priKeyBytes.size());
    const uint32_t hashCode = HashForTest(priKeyBytes.data(), priKeyBytes.size());
    for (uint32_t i = 0; i < entryCount; ++i) {
        std::array<uint8_t, 8> secKeyBytes{};
        std::array<uint8_t, 16> valueBytes{};
        uint64_t encoded = (static_cast<uint64_t>(i) << 32U) | (i ^ 0x9e3779b9U);
        memcpy_s(secKeyBytes.data(), secKeyBytes.size(), &encoded, sizeof(encoded));
        memcpy_s(valueBytes.data(), valueBytes.size(), &encoded, sizeof(encoded));
        BinaryData secKey(secKeyBytes.data(), secKeyBytes.size());
        BinaryData value(valueBytes.data(), valueBytes.size());
        ASSERT_EQ(kMapTable->Put(hashCode, priKey, secKey, value), BSS_OK);
    }
    if (mode == "slice") {
        ForceFlushToSlice();
    } else if (mode == "lsm") {
    ...
    std::vector<uint64_t> openNs;
    std::vector<uint64_t> firstNs;
    std::vector<uint64_t> totalNs;
    openNs.reserve(rounds);
    firstNs.reserve(rounds);
    totalNs.reserve(rounds);
    for (uint32_t round = 0; round <= rounds; ++round) {
        auto begin = std::chrono::steady_clock::now();
        auto *iterator = kMapTable->EntryIterator(hashCode, priKey);
        ...
        if (round != 0) {
            openNs.emplace_back(std::chrono::duration_cast<std::chrono::nanoseconds>(opened - begin).count());
            firstNs.emplace_back(std::chrono::duration_cast<std::chrono::nanoseconds>(first - begin).count());
            totalNs.emplace_back(std::chrono::duration_cast<std::chrono::nanoseconds>(done - begin).count());
        }
    }
    auto median = [](std::vector<uint64_t> values) {
        std::sort(values.begin(), values.end());
        return values[values.size() / 2];
    };
    std::cout << "SUBTABLE_OPEN_BENCH mode=" << mode << " entries=" << entryCount << " rounds=" << rounds
              << " open_us=" << median(openNs) / 1000.0 << " first_us=" << median(firstNs) / 1000.0
              << " total_us=" << median(totalNs) / 1000.0 << std::endl;
}
