...
public:
    ...
    size_t SizeHint() const override
    {
        size_t result = mCurrentElement == nullptr ? 0 : 1;
        for (size_t index = mCurrentIterator; index < mIterators.size(); ++index) {
            result += mIterators[index]->SizeHint();
        }
        return result;
    }
