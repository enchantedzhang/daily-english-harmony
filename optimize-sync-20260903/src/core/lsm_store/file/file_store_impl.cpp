...
KeyValueIteratorRef LsmStore::CreateMergingIterator(
    ...
    std::function<std::vector<FileMetaDataRef>(const Level &level)> filesGetter, bool reverseOrder, bool sectionRead,
...

{
    ...
    auto filesGetter = [prefixKey, keyGroup, current](const Level &level) -> std::vector<FileMetaDataRef> {
