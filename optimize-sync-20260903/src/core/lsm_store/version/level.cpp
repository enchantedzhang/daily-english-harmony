...
std::vector<FileMetaDataRef> Level::GetFilesContainingStateId(uint32_t stateId) const
{
    std::vector<FileMetaDataRef> result;
    FileMetaDataGroupRef curGroup = mFileMetaDataGroups.empty() ? nullptr : mFileMetaDataGroups.at(0);
    while (curGroup != nullptr) {
...
bool Level::IsPrefixKeyWithinFile(const FileMetaDataRef &fileMetaData, const Key &prefixKey) const
{
...
uint32_t Level::FindFile(const std::vector<FileMetaDataRef> &files, const Key &key) const
{
    uint32_t left = 0;
