...
public:
    ...
    inline const std::vector<FileMetaDataGroupRef> &GetFileMetaDataGroups() const
    {
        return mFileMetaDataGroups;
    }
    ...
    std::vector<FileMetaDataRef> GetFilesContainingPrefixKey(const Key &prefixKey, uint32_t keyGroup,
                                                             bool checkOrderRange) const;

    std::vector<FileMetaDataRef> GetFilesContainingStateId(uint32_t stateId) const;
...
private:
    inline FileMetaDataGroupRef NextFileMetaDataGroup(const FileMetaDataGroupRef &group) const
    {
        return (group->GetRight() != nullptr) ? group->GetRight() : group->GetDown();
    }

    inline bool IsInclusiveKeyWithinFile(const FileMetaDataRef &fileMetaData, const Key &key) const
    {
    ...
    inline void GetOverlappingFiles(const std::vector<FileMetaDataRef> &files, const Key &key, bool checkOrderRange,
                                    std::vector<FileMetaDataRef> &result) const
    {
        for (auto &file : files) {
    ...
    bool IsPrefixKeyWithinFile(const FileMetaDataRef &fileMetaData, const Key &prefixKey) const;

    uint32_t FindFile(const std::vector<FileMetaDataRef> &files, const Key &key) const;
