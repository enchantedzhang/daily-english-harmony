...
KeyValueIteratorRef DataBlock::SubIterator(const Key &startKey, const Key &endKey, bool reverseOrder)
{
    ...
    // find entry range [leftIndex, rightIndex] by start key and end key.
    ...
    if (numSecKey == 1) {
        ...
        // multi secondary keys, binary search left index and right index.
        leftIndex = BinarySearch(keyValueInfo, startKey, 0, numSecKey);
        if (leftIndex == numSecKey) {
            return nullptr;
        }
        rightIndex = BinarySearch(keyValueInfo, endKey, leftIndex, numSecKey) - 1;
        if (rightIndex < leftIndex) {
            return nullptr;
        }
    }
    // lazy iterator: decode key/value on demand to avoid materializing the whole range into a vector.
    return std::make_shared<DataBlockSubIterator>(std::move(keyValueInfo), leftIndex, rightIndex, reverseOrder);
}
