...
{
    // get value from blob store. Only build the blob-resolve closure when a blob store is configured
    // (KV-separate); for inline values ToBlobValue is a no-op, so the per-open closure + shared_from_this
    // allocation is avoided in the common case.
