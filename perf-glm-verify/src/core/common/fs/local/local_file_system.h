...
public:
    ...
    BResult Open(int flags) override
    {
        ...
        // hint the kernel to aggressively readahead for sequential prefix scans (no-op on failure).
        posix_fadvise(mFd, 0, 0, POSIX_FADV_SEQUENTIAL);
    ...
    BResult Read(uint8_t *buffer, uint64_t count, int64_t offset) override
    {
        ...
        // robust pread loop: handle short reads and EINTR (signal interruption).
        uint64_t totalRead = 0;
        while (totalRead < count) {
            ssize_t result = pread(mFd, buffer + totalRead, count - totalRead, offset + static_cast<int64_t>(totalRead));
            ...
            if (UNLIKELY(result == 0)) {
                break;  // EOF before reading all bytes
            }
            totalRead += static_cast<uint64_t>(result);
        }
        if (UNLIKELY(totalRead != count)) {
            LOG_ERROR("Pread short read, file: " << mFilePath->ExtractFileName() << ", expected:" << count
                                                 << ", actual:" << totalRead << ", offset:" << offset);
            return BSS_IO_ERR;
        }
        return BSS_OK;
    }
